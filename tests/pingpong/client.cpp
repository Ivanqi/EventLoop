...
class Client
{
    ...
    public:
        Client(EventLoop *loop, const InetAddress& serverAddr, int blockSize, int sessionCount, int timeout, int threadCount)
            :loop_(loop), threadPool_(loop, "pingpong_client"), sessionCount_(sessionCount), timeout_(timeout)
        {
        ...
        void onDisconnect(const TcpConnectionPtr& conn)
        {
            if (numConnected_.decrementAndGet() == 0) {
                ...
                printf("onDisconnect: %ld total bytes read\n", totalBytesRead);
                printf("onDisconnect: %ld total messages read\n", totalMessagesRead);
                
                double tmp = static_cast<double>(totalBytesRead) / static_cast<double>(totalMessagesRead);
                printf("onDisconnect: %f average message size\n", tmp);

                tmp = static_cast<double>(totalBytesRead) / (timeout_ * 1024 * 1024);
                printf("onDisconnect: %f \n", tmp);
